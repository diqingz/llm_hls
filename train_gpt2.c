...
void gpt2_forward(GPT2 *model, int* inputs, int* targets, size_t B, size_t T) {
    ...
    for (int l = 0; l < L; l++) {
        ...
        // Offload the first attention layer to FPGA
        if (l == 0) {
            attention_forward_fpga(l_atty, l_preatt, l_att, l_qkv, B, T, C, NH);
        } else {
            attention_forward(l_atty, l_preatt, l_att, l_qkv, B, T, C, NH);
        }
...
// // Transfer input tensor to FPGA and trigger execution
// if (system("scp input_tensor.bin diqingz2@hal-fpga-x86.ncsa.illinois.edu:~/fpga_gpt2/") != 0) {
// printf("Error: Failed to copy input tensor to FPGA.\n");
// return;
// }

// if (system("ssh diqingz2@hal-fpga-x86.ncsa.illinois.edu 'cd ~/fpga_gpt2 && bash run_attention_fpga.sh'") != 0) {
// printf("Error: Failed to execute FPGA attention layer.\n");
// return;
// }

// // Fetch output tensor from FPGA
// if (system("scp diqingz2@hal-fpga-x86.ncsa.illinois.edu:~/fpga_gpt2/output_tensor.bin .") != 0) {
// printf("Error: Failed to retrieve output tensor from FPGA.\n");
// return;
// }

// // Load the output tensor into the model
...
// if (read_count != B * T * C) {
// printf("Error: Failed to read full output tensor from FPGA\n");
// return;
// }
// fclose(f);
...
int main() {
    ...
    // Allocate memory for generated tokens
    uint64_t rng_state = 1337;
    int B = 1;  // Batch size 1 for inference
    int T = 64; // Sequence length
    int* gen_tokens = (int*)mallocCheck(B * T * sizeof(int));

    // Initialize tokens with the GPT2_EOT token
    for (int i = 0; i < B * T; ++i) {
        gen_tokens[i] = tokenizer.eot_token;
    }

    // Start inference (text generation)
